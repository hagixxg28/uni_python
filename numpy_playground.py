...
def test_func():
    data = np.array(TEST)
    values = get_data_by_max_column_val(data=data,
                                        column_to_compare_by_name=HANDSOMNESS,
                                        columns=[NAME, HANDSOMNESS])
    print(f'{values=}')
...
def extract_column_values(data, column_index,
                          start_index: Optional[int] = 1) -> np.array:
...
def get_data_by_max_column_val(data,
                               column_to_compare_by_name: str,
                               columns: list[str]) -> list:
    max_index = get_max_val_index_in_column(
        data=data,
